...
scope[current.resolveAs||'$resolve']=locals;link(scope);}};}})(window,window.angular);var duScrollDefaultEasing=function(x){'use strict';if(x<0.5){return Math.pow(x*2,2)/2;}
...
if(!targetId)return;var timeoutPromise=$timeout(function(){var spy=new Spy(targetId,$scope,$element,-($attr.offset?parseInt($attr.offset,10):duScrollOffset));spyAPI.addSpy(spy);$scope.$on('$locationChangeSuccess',spy.flushTargetCache.bind(spy));var deregisterOnStateChange=$rootScope.$on('$stateChangeSuccess',spy.flushTargetCache.bind(spy));$scope.$on('$destroy',function(){spyAPI.removeSpy(spy);deregisterOnStateChange();});},0,false);$scope.$on('$destroy',function(){$timeout.cancel(timeoutPromise);});}};}]);'use strict';angular.module('angular-parallax',[]).directive('parallax',['$window',function($window){return{restrict:'A',scope:{parallaxRatio:'@',parallaxVerticalOffset:'@',parallaxHorizontalOffset:'@',},link:function($scope,elem,attrs){var setPosition=function(){var calcValY=$window.pageYOffset*($scope.parallaxRatio?$scope.parallaxRatio:1.1);if(calcValY<=$window.innerHeight){var topVal=(calcValY<$scope.parallaxVerticalOffset?$scope.parallaxVerticalOffset:calcValY);elem.css('transform','translate('+$scope.parallaxHorizontalOffset+'px, '+topVal+'px)');}};setPosition();angular.element($window).bind("scroll",setPosition);angular.element($window).bind("touchmove",setPosition);}};}]).directive('parallaxBackground',['$window',function($window){return{restrict:'A',transclude:true,template:'<div ng-transclude></div>',scope:{parallaxRatio:'@',},link:function($scope,elem,attrs){var setPosition=function(){var calcValY=(elem.prop('offsetTop')-$window.pageYOffset)*($scope.parallaxRatio?$scope.parallaxRatio:1.1);elem.css('background-position',"50% "+calcValY+"px");};angular.element($window).bind('load',function(e){setPosition();$scope.$apply();});angular.element($window).bind("scroll",setPosition);angular.element($window).bind("touchmove",setPosition);}};}]);window.matchMedia||(window.matchMedia=function(){"use strict";var styleMedia=(window.styleMedia||window.media);if(!styleMedia){var style=document.createElement('style'),script=document.getElementsByTagName('script')[0],info=null;style.type='text/css';style.id='matchmediajs-test';script.parentNode.insertBefore(style,script);info=('getComputedStyle'in window)&&window.getComputedStyle(style,null)||style.currentStyle;styleMedia={matchMedium:function(media){var text='@media '+media+'{ #matchmediajs-test { width: 1px; } }';if(style.styleSheet){style.styleSheet.cssText=text;}else{style.textContent=text;}
return info.width==='1px';}};}
return function(media){return{matches:styleMedia.matchMedium(media||'all'),media:media||'all'};};}());(function(){'use strict';angular.module('ngTooltip',[]).directive('tooltip',tooltip);function tooltip($rootScope){var directive={restrict:'A',link:link}
return directive;function link(scope,element,attrs){var tooltip,container,span,arrow={};var enter,leave,move;scope.$on('ngTooltip:hide',function(){leave();element.off('mouseenter');element.off('mouseleave');element.off('mousemove');});scope.$on('ngTooltip:show',function(){element.on('mouseenter',enter);element.on('mouseleave',leave);element.on('mousemove',move);});enter=function(){if(container){leave();}
container=angular.element('<div></div>').addClass('tooltip').append('<span>'+attrs.tooltip+'</span><div class="arrow"></div>');$('body').append(container);tooltip=angular.element(container)[0];span=tooltip.querySelector('span');arrow=tooltip.querySelector('.arrow');arrow.style.left=+$(span).width()/ 2+'px';span.style.width=+$(span).width()+15+'px';};leave=function(){$('body').find(container).remove();tooltip,container,span,arrow={};};move=function(e){var x=(e.clientX- +$(span).width()/ 2-5)+'px',y=(e.clientY-45)+'px';tooltip.style.display='block';tooltip.style.width=+$(span).width()+15+'px';tooltip.style.top=y;tooltip.style.left=x;};element.on('mouseenter',enter);element.on('mouseleave',leave);element.on('mousemove',move);}}})();